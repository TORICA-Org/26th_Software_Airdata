...
void extractLogData(int readnum) {
  const int Bico_data_num = 54;  // 正常な場合のデータ受信数
  ...
  if (readnum == Bico_data_num) {
    ...
    // 3回目の受信 14個
    fslg_is_alive = static_cast<bool>(Bico_UART.UART_data[24]);
    ...
    data_fslg_bno_cal_system = static_cast<uint8_t>(Bico_UART.UART_data[44]);
    data_fslg_bno_cal_gyro = static_cast<uint8_t>(Bico_UART.UART_data[45]);
    data_fslg_bno_cal_accel = static_cast<uint8_t>(Bico_UART.UART_data[46]);
    data_fslg_bno_cal_mag = static_cast<uint8_t>(Bico_UART.UART_data[47]);
    under_is_alive = static_cast<bool>(Bico_UART.UART_data[48]);
