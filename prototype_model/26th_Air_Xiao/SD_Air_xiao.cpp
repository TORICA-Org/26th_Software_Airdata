...
void flashHeader() {
  ...
  for (int i = 0; i < 4 /* case0~3まで実行 */; i++) {
    ...
    switch (i) {
      ...
      case 3: // 13個
        {
          str[0] = "data_fslg_bno_accx_mss,data_fslg_bno_accy_mss,data_fslg_bno_accz_mss,"; // 3個
          str[1] = "data_fslg_bno_calibration,under_is_alive,data_under_bmp_pressure_hPa,"; // 3個
